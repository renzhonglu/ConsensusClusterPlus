...
\textit{Consensus Clustering} \cite{monti} is a method that provides quantitative evidence for determining the number
 and membership of possible clusters within a dataset, such microarray gene expression.  This method has gained popularity
 in cancer genomics, where new molecular subclasses of disease have been discovered \cite{hayes,verhaak}.  
 The \textit{Consensus Clustering} method involves subsampling from a set of items, such as microarrays, determines clusterings of specified cluster counts (\textit{k}).  Then, pairwise \textit{consensus} values, the proportion that two items occupied the same cluster out of the number of 
...
The \textit{consensus matrix} is summarized in several graphical summaries that enable a user to decide upon a reasonable cluster number and membership.  
A web-based version of \textit{Consensus Clustering} is publicly available \cite{gp}.  For a formal description, see \cite{monti}.
\\
\\
\texttt{ConsensusClusterPlus} implements the \textit{Consensus Clustering} method in \textit{R} and extends it with
...
The first step is to gather some data for cluster analysis. This data could be the result of an experiment such as a mRNA expression microarray or
...
which can be optionally written to a CSV file depending on the writeTable option.

\subsection{Consensus Matrices}
The first graphic shows the consensus color legend.
\\
<<echo=F,results=tex>>=
cat("\\includegraphics[width=60mm]{",title,"/consensus001.png}",sep="")
@
\\
...
ordered by the consensus clustering which is depicted as a dendrogram atop the heatmap.  To aid analysis, the cluster membership are marked by colored rectangles between
...
\\
<<echo=F,results=tex>>=
cat("\\includegraphics[width=60mm]{",title,"/consensus002.png}",sep="")
cat("\\includegraphics[width=60mm]{",title,"/consensus003.png}",sep="")
@
\\
<<echo=F,results=tex>>=
cat("\\includegraphics[width=60mm]{",title,"/consensus004.png}",sep="")
cat("\\includegraphics[width=60mm]{",title,"/consensus005.png}",sep="")
@
\\
...
\\
<<echo=F,results=tex>>=
cat("\\includegraphics[width=60mm]{",title,"/consensus007.png}",sep="")
@
\\
\subsection{CDF Change Plot}
...
\\ 
<<echo=F,results=tex>>=
cat("\\includegraphics[width=60mm]{",title,"/consensus008.png}",sep="")
@
\\
...
\\
<<echo=F,results=tex>>=
cat("\\includegraphics[width=60mm]{",title,"/consensus009.png}",sep="")
@
\\
...
\\
<<echo=F,results=tex>>=
cat("\\includegraphics[width=60mm]{",title,"/icl003.png}",sep="")
@
\\
...
at a particular \textit{k}.  These values are depicted in barplots for each \textit{k}.  Samples are stacked bars.  \textit{Item-consensus} values are indicatged by the heights of the colored portion of 
the bars, whose color corresponds to the common color scheme.  A bars rectangles are ordered
by increasing value from bottom to top.  The asterisks at the top indicate the consensus cluster for each item.
\\
<<echo=F,results=tex>>=
cat("\\includegraphics[width=60mm]{",title,"/icl001.png}",sep="")
@
\\
...
cluster or if it share high consensus to multiple clusters (large rectangles in a column of multiple colors), suggesting that it is an unstable or "unpure" member.  These values could be used to
...
\\
\\
\subsection{Additional options for \texttt{ConsensusClusterPlus} function}

\begin{itemize}

\item \textbf{clusterAlg} This option specifies the type of clustering algorithm to use: "hc" for heirarchical clustering, and "km" for kmeans.  Alternatively, one can
supply their own clustering hook, which should accept a distance matrix and a cluster number as its arguments and returns vector of cluster assignments having the same order as the distance matrix columns.
...
Note: pearson is suggested for heirarchical clustering and euclidean for kmeans.
...
\item \textbf{weightsFeature} numerical vector of weights for sampling features.  See help(sample) for further details. 
\item \textbf{weightsItem} numerical vector of weights for sampling items.  See help(sample) for further details.
...
\bibitem{hayes} Hayes, D.N, Monti, S., Parmigiani, G. et al. (2006) Gene Expression Profiling Reveals Reproducible Human Lung Adenocarcinoma Subtypes in Multiple Independent Patient Cohorts. Journal of Clinical Oncology, Vol 24, No 31: pp. 5079$-$5090.
